...
async def salvar_upload(file: UploadFile | None) -> str | None:
    ...
    if settings.use_supabase and settings.supabase_url and settings.supabase_access_key:
        try:
            from supabase import create_client

            object_path = f"monitoramentos/{filename}"
            client = create_client(settings.supabase_url, settings.supabase_access_key)
